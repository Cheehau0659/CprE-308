...
void *producer(void *arg)
{
  ...
  while (!producer_done)
  {
    ...
    while(supply != 0) {
      // wait for the consumer sig while there is still supply
      pthread_cond_wait(&producer_cv, &mut); 
    }
    supply += 10; // replenish supply
    pthread_cond_broadcast(&consumer_cv); // signal all the consumers
  
    if(num_cons_remaining == 0) {
      producer_done = 1; // done if all consumers fed
    }
